...
def safe_worker_task(model, simulations, puct, state_factory, temperature, temperature_cutoff):
    """
    Runs the self-play game inside a safe try-except block.
    Initializes MCTS *inside* the worker to avoid pickling complex objects across process boundaries.
    """
    try:
        with torch.no_grad():
            mcts_worker = MCTS(model, simulations=simulations, puct=puct)
            return self_play_game(mcts_worker, state_factory, temperature, temperature_cutoff)
    except Exception as e:
        print(f"\n[CRITICAL WORKER CRASH] True error caught before serialization failure:")
        traceback.print_exc()
        raise e
# ──────────────────────────────────────────────────────────────────────────────
